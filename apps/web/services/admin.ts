...
export async function getAllAdminTerrains(params?: AdminListParams) {
  const firstPage = await getAdminTerrains({ ...params, page: 1, limit: 100 });
  ...
  const remainingPages = await Promise.all(
    Array.from({ length: firstPage.meta.totalPages - 1 }, (_, index) =>
      getAdminTerrains({ ...params, page: index + 2, limit: 100 })
    )
  );
