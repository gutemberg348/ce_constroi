...
export function MediaGallery({ items, title }: { items: MediaGalleryItem[]; title: string }) {
  ...
  return (
    <>
      <div className="overflow-hidden rounded-[8px] border border-[var(--line)] bg-[var(--panel)] p-2 sm:p-3">
        <button
        ...
        >
          <PrivacyImage
            alt={selected.alt}
            className="h-64 w-full object-cover transition duration-500 group-hover:scale-[1.02] sm:h-96 lg:h-[520px]"
            src={selected.src}
          />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/86 p-3 sm:p-6" role="dialog" aria-modal="true">
          <div className="relative grid max-h-full w-full max-w-6xl gap-3">
            <div className="flex items-center justify-between gap-3 text-white">
              <div>
                <p className="text-sm uppercase text-white/60">{title}</p>
                <h2 className="text-lg font-semibold">{selected.label ?? `Foto ${selectedIndex + 1}`}</h2>
              </div>
              <button
            ...
            <div className="relative overflow-hidden rounded-[8px] bg-black">
              <PrivacyImage alt={selected.alt} className="max-h-[76vh] w-full object-contain" src={selected.src} />
              {gallery.length > 1 ? (
                <>
                  <button
